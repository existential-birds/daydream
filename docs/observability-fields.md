...
`AttemptObserver._seal_generation` + `_end_generations` (`spans.py`,
`_seal_generation` … `_end_generations`),
...
| `gen_ai.conversation.id` | All backends, attempt observer | `GenerationStartEvent.session_id` (native conversation/session id supplied by the backend event) | attempt | string | – | 0..1 | Observed event field | When the backend event supplies it | – | Span attribute | metadata | metadata | `test_observability_backend_protocols.py` conversation asserts | verified | Applicable; native conversation/session identity, never substituted for Daydream run/trajectory/response ids |
...
| `daydream.models` / `daydream.providers` | ordered observed identity list | distinct ordered observed model/provider values | attempt | array(string) | – | 0..2 | Observed lists, admitted through the 16-entry whole-list-overflow rule | When observed | – | Span attributes | metadata | metadata | T1/T5 + `test_observability_runtime.py` emission asserts | verified | Conditional; max 16 with overflow omitting the whole list (fixed diagnostic, never a partial list) |
...
| field name | source backend/event | source authority/provenance | owning span | type | unit | cardinality | derivation | completeness | capture/redaction | generic OTLP disposition | HoneyHive canonical destination | LangSmith native destination | offline test node | live evidence status | applicability and omission reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `gen_ai.operation.name=execute_tool`, `gen_ai.tool.name`, `gen_ai.tool.call.id` | ToolStartEvent | exact tool start identity | tool | string | – | 1 | Observed | Always | – | Span attributes | tool classification | `run_type=tool` | `test_codex_multi_turn_replay_yields_two_tool_spans_and_isolated_turns` | verified | Applicable; name ASCII≤128 profile |
