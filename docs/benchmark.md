...
### Machine-readable run objectives (`objective`) and suite aggregation (`aggregate`)

Each completed Harbor run resolves to a single machine-readable **objective**:
count-derived micro-metrics (TP/FP/FN pooled to micro precision/recall/F1, plus
task/clean/scored/infra counts) bound to the run's full **compatibility
identity** (review-profile digest, runtime digest, reviewer/judge identity,
verifier template, threshold, attempts). Resolve one exact run with

```bash
daydream benchmark objective <dir> --run-id <id> [--json PATH|-]
```

`<dir>` is the private workspace and `--run-id` the exact ledgered run id.
`--json PATH` writes the strict, machine-readable objective JSON to `PATH`;
`--json -` routes the pure JSON blob to stdout (the human summary goes to
stderr) so a `jq`/file-redirect consumer sees only the blob. Unscored or
infrastructure-failed runs resolve with no objective rather than a fabricated
zero. The **review-profile digest** (a canonical hash of the exact tested
profile) and the full identity surface appear in the objective blob, and the
run's local provenance echo the same `profile_digest` so a receipt can be
cross-checked against the tested candidate.

Pool several completed runs into one compatible suite objective with

```bash
daydream benchmark aggregate <manifest> [--json PATH|-]
```

`<manifest>` is a suite manifest carrying a `schema_version` and an ordered
list of `{workspace, run_id}` entries. Every entry must resolve and bind a
matching compatibility identity — any differing profile digest, runtime
digest, reviewer/judge identity, verifier template, threshold, or attempts
fails the whole command, as does a missing/incomplete/comparison-ineligible
entry, an infrastructure-failed run, or a duplicated `(workspace, run_id)`
pair. Aggregation **fails closed** — it never returns a silently-subsetted
pool. The pooled objective flattens the per-task rows across every run and
feeds them through the same micro-metric aggregator once, so TP/FP/FN pool to
**micro** precision/recall/F1 and the counts sum across runs — it never
averages per-repository precision/recall/F1. `--json -` emits the strict suite
objective (a stable `experiment_id`, the shared `profile_digest`, and the
privacy-safe identity) to stdout while the digest/identity summary lands on
stderr.

**Explicit-only trust boundary.** Harbor benchmarks only an explicitly
trusted candidate: the control-plane `DAYDREAM_REVIEW_PROFILE_CANDIDATE`
value (or the packaged default) parsed and validated *before* the run. It
ignores ambient user profiles and the target repository's own
`.daydream.toml`/profiles — a benchmarked repo can never configure its own
evaluator. The test profile is resolved from the candidate before any review
starts; an invalid candidate aborts the run with nothing written.

**No automatic tuning.** Harbor runs each reference as-is and has **no**
automatic search or self-tuning loop of its own — it never re-runs a case
toward a better score, never iterates thresholds, and contains no
hill-ward adjustment of the parameters it measures. The stable
`objective`/`aggregate` seams and the candidate-profile channel are exactly
that: read-only, compatibility-bounded enablers a future improvement agent
could drive. They are not a searcher or tuner shipped today. `#885`/`#888`
only make the comparison stable and machine-readable; turning them into an
adaptive search-and-tune loop is a separate, future cutover.
