...
class ExplorationLivePanel:
    """Per-subagent live panel for the EXPLORE phase (D-15).

    Renders one row per active specialist (pattern-scanner,
    dependency-tracer, test-mapper) with a throbber that transitions to
    done/failed when the corresponding subagent finishes. Used as a
    context manager so that abnormal exits flip any leftover ``pending``
    rows to ``failed`` for visibility.

    Args:
        console: Rich Console instance for output.
        tier: Either ``"single"`` (one row) or ``"parallel"`` (three rows).

    """

    _SINGLE_ROWS = ("dependency-tracer",)
    _PARALLEL_ROWS = ("pattern-scanner", "dependency-tracer", "test-mapper")

    def __init__(self, console: Console, tier: str) -> None:
        row_keys: tuple[str, ...]
        if tier == "single":
            row_keys = self._SINGLE_ROWS
        else:
            row_keys = self._PARALLEL_ROWS
        self._console = console
        self._tier = tier
        self.row_keys: tuple[str, ...] = row_keys
        self.states: dict[str, str] = {k: "pending" for k in row_keys}
        self.errors: dict[str, str | None] = {k: None for k in row_keys}
        self._frame = 0
        self._live: Live | None = None
    ...
    def _build_panel(self) -> Panel:
        ...
        for key in self.row_keys:
            ...
            if state == "running":
                icon = Text(
                    PARALLEL_SPINNER_FRAMES[self._frame % len(PARALLEL_SPINNER_FRAMES)],
                    style=STYLE_CYAN,
                )
                msg = Text("exploring...", style=STYLE_FG)
            elif state == "done":
                icon = Text("\u2713", style=STYLE_GREEN)
                msg = Text("done", style=STYLE_DIM)
            elif state == "failed":
                icon = Text("\u2717", style=STYLE_RED)
                err = self.errors.get(key) or "failed"
                msg = Text(err, style=STYLE_DIM)
            else:
                icon = Text("\u00b7", style=STYLE_DIM)
                msg = Text("pending", style=STYLE_DIM)

            table.add_row(icon, Text(key, style=STYLE_PURPLE), msg)

        return Panel(
            table,
            title=Text("EXPLORE", style=STYLE_BOLD_PINK),
            border_style=STYLE_PINK,
            box=box.ROUNDED,
            padding=(0, 1),
        )
    ...
    def mark_start(self, name: str) -> None:
        """Transition a row to ``running`` state."""
        if name in self.states:
            self.states[name] = "running"
            self._refresh()

    def mark_done(self, name: str) -> None:
        """Transition a row to ``done`` state."""
        if name in self.states:
            self.states[name] = "done"
            self._refresh()

    def mark_failed(self, name: str, error: str | None = None) -> None:
        """Transition a row to ``failed`` state with an optional error message."""
        if name in self.states:
            self.states[name] = "failed"
            self.errors[name] = error
            self._refresh()

    def __enter__(self) -> "ExplorationLivePanel":
        self._live = Live(
            self._build_panel(),
            console=self._console,
            refresh_per_second=8,
    ...
    def __exit__(self, exc_type, exc, tb) -> None:  # type: ignore[no-untyped-def]
        # Any row still pending at exit is treated as a crash so the user can see it.
        for key in self.row_keys:
            if self.states[key] == "pending":
                self.states[key] = "failed"
                if self.errors[key] is None:
                    self.errors[key] = "no result"
        if self._live is not None:
            self._live.update(self._build_panel())
            self._live.stop()
            self._live = None
