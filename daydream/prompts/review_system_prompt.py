...
Any non-Python text in your response causes a SyntaxError. Use # comments for explanations.
...
## Available Functions
    ...
    # Large file handling (for files >10k tokens, use get_file_slice() or batch into llm_query())
...
plain markdown text (NOT wrapped in code blocks).
    ...
    # Option 1: Direct string (use for simple reports)
    ...
    report += "- [HIGH] src/auth.py:45 - SQL injection in login query\\n"
    report += "- [MEDIUM] src/api.py:120 - Missing rate limiting\\n"
    FINAL_VAR("report")  # Uses the report variable you built up

---

## Rules

1. **For large files (>10k tokens)** — use `get_file_slice()` or batch content into `llm_query()`
...
On your FIRST turn, execute this code to determine review scope:
...
After this first step, proceed with the "Probe → Filter → Batch → Aggregate" strategy.
'''
...
"""
    ...
    # Append PR context to the base prompt
