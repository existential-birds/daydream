...
def render_run_info_block(
    ...
    Args:
        ...
        daydream_version: Version string for the footer (M3).
    ...
    try:
        if not trajectory_paths:
            return _render_fallback(mode_label, daydream_version)
        ...
        if not agg.phases:
            return _render_fallback(mode_label, daydream_version)
        return _render(agg, mode_label, daydream_version)
...
def _render(agg: _RunAgg, mode_label: str, daydream_version: str) -> str:
    ...
    lines.append("")
    lines.append(_render_footer(daydream_version))
    return "\n".join(lines)
...
def _render_footer(daydream_version: str) -> str:
    """Small-text footer naming the daydream version (M3)."""
    return f"<sub>Generated by daydream v{daydream_version}</sub>"


def _render_fallback(mode_label: str, daydream_version: str) -> str:
    """M9: degrade to today's single Mode line plus a 'run details unavailable' note.

    Footer still included so a reader can tell which daydream version
    produced the comment even when trajectory parsing failed.
    """
    return (
        f"- **Mode:** {mode_label}\n"
        f"\n"
        f"{_FALLBACK_NOTE}\n"
        f"\n"
        f"{_render_footer(daydream_version)}"
    )
