...
def _is_clean_review(classified: _ClassifiedIssues, approve_on_clean: bool) -> bool:
    ...
    blocking severity. Severity is matched case-insensitively — the findings
    schema permits any string, so 'High'/'CRITICAL'/'blocker' must block too
    — while ``None`` (a model that omitted severity) deliberately does not.
    """
    if not approve_on_clean:
        return False
    return not any(
        issue.severity is not None and issue.severity.lower() in ("high", "medium")
...
def build_payload(
    ...
    *,
    ...
    if not clean:
        # COMMENT payloads keep the reviewed SHA so inline-comment line
        # anchors stay valid. An approval must not be pinned to
        # ``pr.head_sha``: the deep flow posts the review before its fix
        # cycle, and tying the APPROVE to that pre-fix head would let a later
        # fix commit stale-dismiss it (#343). Omitting ``commit_id`` makes
        # GitHub anchor the review to the PR's most recent commit.
