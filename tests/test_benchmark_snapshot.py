...
def _seed_origin(tmp_path: Path) -> str:
    ...
    base2_sha = seed_commit(repo, "base2")
...
def _seed_two_pr_origin(tmp_path: Path) -> tuple[str, str, str]:
    ...
    seed_write(repo, "dev.py", "DEV = 1\n")
    dev_tip = seed_commit(repo, "dev1")
    repo.joinpath("dev.py").write_text("DEV = 2\n")
    _git(repo, "add", "dev.py")
    pr2_head = seed_commit(repo, "dev2")
    bare = tmp_path / "origin.git"
    bare.mkdir(parents=True, exist_ok=True)
    _git(bare, "init", "--bare")
    _git(repo, "remote", "add", "origin", str(bare))
...
def _seed_rename_origin(tmp_path: Path) -> tuple[str, str, str]:
    ...
    repo.mkdir()
    _git(repo, "init", "-b", "main")
    seed_write(repo, "old.py", "def old() -> int:\n    return 1\n")
    authoring_sha = seed_commit(repo, "author old.py")
...
def _seed_anchor_origin(tmp_path: Path, *, pr: int = 1) -> tuple[str, str, str]:
    ...
    repo.mkdir()
    _git(repo, "init", "-b", "main")
    seed_write(repo, "a.py", "def a() -> int:\n    return 1\n")
    authoring_sha = seed_commit(repo, "author a.py")
    repo.joinpath("a.py").write_text("def a() -> int:\n    return 2\n")
    _git(repo, "add", "a.py")
    head_sha = seed_commit(repo, "edit a.py")
...
def _seed_double_rename_origin(tmp_path: Path, *, pr: int = 1) -> tuple[str, str, str]:
    ...
    repo.mkdir()
    _git(repo, "init", "-b", "main")
    seed_write(repo, "old1.py", "def old1() -> int:\n    return 1\n")
    seed_write(repo, "old2.py", "def old2() -> int:\n    return 2\n")
    authoring_sha = seed_commit(repo, "author old1 old2")
    _git(repo, "mv", "old1.py", "new1.py")
    _git(repo, "mv", "old2.py", "new2.py")
    head_sha = seed_commit(repo, "rename both")
...
def _seed_rich_origin(tmp_path: Path) -> tuple[str, str, str, str, str]:
    ...
    repo.mkdir()
    _git(repo, "init", "-b", "main")
    seed_write(repo, "readme.txt", "hello rich\n")
    ...
    base_sha = seed_commit(repo, "rich base")
    ...
    _git(repo, "add", "payload.bin")
    head_sha = seed_commit(repo, "rich head")
...
def _seed_facts_origin(tmp_path: Path) -> tuple[str, str, str, str]:
    ...
    base2_sha = seed_commit(repo, "base2")
    seed_write(repo, "beyond.py", "BEYOND = 3\n")
    base3_sha = seed_commit(repo, "base3")
    _git(repo, "checkout", "--detach", base2_sha)
    seed_write(repo, "old.py", "def old() -> int:\n    return 1\n")
    seed_write(repo, "feature.py", "FEATURE = 1\n")
    authoring_sha = seed_commit(repo, "author old.py + feature.py")
    _git(repo, "mv", "old.py", "new.py")
    repo.joinpath("feature.py").write_text("FEATURE = 2\n")
    _git(repo, "add", "feature.py")
    (repo / "blob.bin").write_bytes(b"\x00\x01\x02\x03binary")
    _git(repo, "add", "blob.bin")
    head_sha = seed_commit(repo, "rename + edit + binary")
...
def _seed_delta_origin(tmp_path: Path) -> tuple[str, str, str, dict[str, str]]:
    ...
    seed_commit(repo, "base1")
    seed_write(repo, "base.py", "BASE = 2\n")
    seed_write(repo, "wide.py", "".join(f"line{i}\n" for i in range(1, 11)))
    seed_write(repo, "renamed.txt", "r\n")
    seed_write(repo, "deleted.txt", "d\n")
    base2_sha = seed_commit(repo, "base2")
    ...
    edit_head = seed_commit(repo, "edit wide.py line 10")

    _git(repo, "checkout", "--detach", base2_sha)
    _git(repo, "mv", "renamed.txt", "renamed2.txt")
    rename_head = seed_commit(repo, "rename renamed.txt")

    _git(repo, "checkout", "--detach", base2_sha)
    _git(repo, "rm", "deleted.txt")
    delete_head = seed_commit(repo, "delete deleted.txt")

    _git(repo, "checkout", "--detach", base2_sha)
    (repo / "bin.dat").write_bytes(b"\x00\x01\x02binary")
    _git(repo, "add", "bin.dat")
    binary_head = seed_commit(repo, "add bin.dat")

    _git(repo, "checkout", "--detach", base2_sha)
    repo.joinpath("orphan.txt").write_text("o\n")
    _git(repo, "add", "orphan.txt")
    orphan_sha = seed_commit(repo, "orphan")
