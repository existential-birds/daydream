...
def test_fake_gh_rejects_credential_helper_without_operation(tmp_path, monkeypatch):
    install_fake_gh(tmp_path, monkeypatch)
    proc = git_ops._run_gh(tmp_path, ["auth", "git-credential"])   # no get/store/erase
    assert proc.returncode != 0


def test_fake_gh_rejects_credential_helper_missing_stdin(tmp_path, monkeypatch):
    install_fake_gh(tmp_path, monkeypatch)
    proc = git_ops._run_gh(tmp_path, ["auth", "git-credential", "get"], input_text="")
    assert proc.returncode != 0


def test_fake_gh_accepts_valid_credential_helper_get(tmp_path, monkeypatch):
    install_fake_gh(tmp_path, monkeypatch)
...
def test_fake_gh_requires_helper_fragment_in_git_ls_remote(tmp_path, monkeypatch):
    install_fake_gh(tmp_path, monkeypatch)
