...
def test_e2e_single_phase_claude_renders_full_block() -> None:
    ...
    # Footer.
    assert out.rstrip().endswith("<sub>Generated by daydream v0.14.0</sub>")
...
def test_e2e_multi_phase_renders_per_phase_table_rows() -> None:
    ...
    """
    out = render_run_info_block([_MULTI_PHASE_CLAUDE], "deep review", "0.14.0")
...
def test_e2e_mixed_models_renders_mixed_label() -> None:
    ...
    """
    out = render_run_info_block([_MIXED_MODELS], "comment", "0.14.0")
...
def test_e2e_codex_cached_tokens_show_in_rollup() -> None:
    ...
    """
    out = render_run_info_block([_CODEX_WITH_CACHED], "feedback 99", "0.14.0")
...
def test_e2e_unknown_model_emits_named_footnote() -> None:
    ...
    """
    out = render_run_info_block([_UNKNOWN_MODEL], "ttt", "0.14.0")
    assert "- **Cost:** —" in out
...
def test_e2e_corrupted_trajectory_falls_back_to_single_mode_line() -> None:
    ...
    Covers M9 + K8 end-to-end. Fallback block has the Mode line, the
    'run details unavailable' note, the version footer — and crucially
    NOT the per-phase table (no half-rendered output).
    """
    out = render_run_info_block([_CORRUPTED], "ttt", "0.14.0")
    assert "- **Mode:** ttt" in out
    assert "*run details unavailable*" in out
    assert "<sub>Generated by daydream v0.14.0</sub>" in out
