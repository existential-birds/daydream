...
The seed family (``SEED_ENV`` + ``seed_write`` + ``seed_commit``) is what makes
...
from tests.harness.git_helpers import git, init_repo, seed_commit, seed_write
...
def test_seed_family_produces_pinned_shas(tmp_path: Path) -> None:
    ...
    base1 = seed_commit(repo, "base1")
    seed_write(repo, "base.py", "BASE = 2\n")
    base2 = seed_commit(repo, "base2")
...
def test_seed_write_stages_binary_content(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    init_repo(repo)
    seed_write(repo, "blob.bin", b"\x00\x01\x02")
    seed_commit(repo, "binary")
