...
- Taskset id / module: `daydream-review` / `daydream_review`
- Pins: `verifiers==0.2.1`, prime-rl `v0.7.0` (see `../train/README.md`)

```bash
cd rl/daydream_review
uv sync
uv run ruff check . && uv run mypy daydream_review tests && uv run pytest
```

## How a rollout works

```text
Taskset.load()          images/manifest.toml (PR snapshots + golden comments), C5-enforced
...
1. **Author manifest entries.** Any repo NOT in `daydream/training/schema/exclusion.txt`:

   Each `[[repos."OWNER/REPO".prs]]` table in `images/manifest.toml` is one
   PR snapshot: `pr_number`, `base_sha`, `head_sha`, `base_ref` (default
   `main`), and optional `golden_comments` tables. The manifest is the single
   source of truth for tasks, image builds, and golden-comment scoring.
...
```bash
uv run python -m daydream_review.fixture /tmp/daydream-rl-smoke/repo
mkdir -p /tmp/daydream-rl-smoke/archive /tmp/daydream-rl-smoke/home
uv run python -m daydream_review.stub_upstream --port 8399 &
...
`rl/daydream_review/pyproject.toml` and `uv.lock`. The gate test
...
cd rl/daydream_review && uv run pytest tests/test_vendored_verifiers_suite.py -q
```
