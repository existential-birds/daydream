"""Build the rollout container images for the ``daydream-review`` environment.
...
Usage::
    ...
    uv run python images/build_images.py --no-base daydream-rl/base:v1.2.3 --manifest ../manifests/train.toml
...
from daydream_review.taskset import _ManifestEntry, _ManifestPR, _repo_slug, load_manifest
...
#: repository" instead of cloning anything (``daydream_review.fixture``).
...
def acquire_mirror(
    ...
    """
    # Key by the manifest repo slug, never by a slug derived
...
def materialize_mirror(entry: _ManifestEntry, ctx: Path, *, red: bool, mirror: Path) -> dict[str, str]:
    ...
    Returns:
        ...
        so that commit's SHA changes and the SHA pinned in the manifest no longer
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the daydream-review rollout images.")
    ...
    if args.only:
        prs = [item for item in prs if item[0] == args.only]
        if not prs:
    ...
    failed: list[str] = []
    try:
        for slug, entry, pr in prs:
            if not pr.base_sha:
                print(f"FAILED {slug}#{pr.pr_number}: manifest entry has no base_sha", file=sys.stderr)
            ...
            try:
                ...
                # baseline does not hide the state of the rest of the manifest. A failed
