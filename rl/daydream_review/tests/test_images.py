...
def _write_manifest(path: Path, entries: list[tuple[str, list[tuple[int, str, str]]]]) -> Path:
    """Write a minimal manifest: (slug, [(pr_number, base_sha, head_sha), ...])."""
    blocks = []
    for slug, snapshots in entries:
        block = (
            f'[repos."{slug}"]\n'
            f'clone_url = "https://github.com/{slug}.git"\n'
            f'image = "daydream-rl/{slug.split("/")[-1]}"\n'
            'test_command = "pytest -q"\n'
            "setup_cmds = []\n"
            'protected_test_paths = ["tests"]\n'
        )
        for pr_number, base_sha, head_sha in snapshots:
            block += (
                f'\n[[repos."{slug}".prs]]\n'
...
def test_main_acquires_upstream_mirror_once_per_slug(
    ...
    # Two PRs of a REAL upstream slug, declared on a temp manifest: the fixture
    # slug's acquire_mirror branch clones from a local fixture build (a temp
    ...
    status = build_images.main(["--manifest", str(manifest), "--only", REFERENCE_SLUG])
    assert status == 0
...
def test_green_baseline_gate_fails_the_build_on_a_red_suite(base_image: str) -> None:
    ...
    # manifest keeps its other PR green and legitimately produces an image. So the invariant is
...
def test_reference_image_builds_with_locked_dependencies(base_image: str) -> None:
    ...
    result = _build(base_image, slug=REFERENCE_SLUG)
    combined = result.stdout + result.stderr
...
def test_build_emits_canonical_argv_for_all_call_sites(
    ...
    # expected argv tail) so the delta between call sites — slug, --red — is
