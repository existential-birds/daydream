...
- **codex backend:** on macOS, the isolated read-only child environment now resolves the real Git binary via `xcrun --find git` in the parent process and prepends its directory to the child's PATH (validated, cached once per process, fail-open to the unchanged PATH with a warning on any resolution failure), so `git` run under `--sandbox read-only` no longer selects the `/usr/bin/git` xcrun shim and Seatbelt no longer sprays xcrun diagnostic noise into shell output. Non-Darwin platforms are behaviorally unchanged (#1122)
