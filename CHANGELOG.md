...
- **observability:** integrate OpenLLMetry with extensible OTLP exporters (#1141)
- trajectory phase attribution + push safety for issue #726 (#1137)
- **adjudication:** hydrated-stage materialization + preview/import pipeline (Closes #1095) (#1133)
- **backends:** snapshot source local branch refs into read-only clones (#1121) (#1131)
- **pr-review:** render linked reviewed-commit line in PR summary review-info block (#1115)
- **archive:** import observations from archive backups for adjudication (#1082) (#1086)
- **hydrate:** corpus hydrate-hub — pinned Hub hydration with dedupe, resumable download, and publication cycle (Closes #982) (#1003)
- **training:** four-stage training pipeline (reward gate → SFT → RFT → GRPO) (Closes #91) (#995)
- **benchmark:** score finding location and severity as reported axes alongside content F1 (#994)
- **harbor:** claude backend support for benchmark agent run gate (Closes #966) (#976)
...
- cut ~3.7k lines from the artifact-visibility surface (#1163)
- **artifacts:** consolidate crash-safe primitives, unify session routing (#1164)
- preserve replayable shell commands in trajectories (#1146)
- **deps:** bump astral-sh/setup-uv from 4.2.0 to 10.0.1 (#1068)
- **deps:** bump actions/checkout from 4.3.1 to 7.0.1 (#1067)
- **deps:** bump actions/download-artifact from 4.3.0 to 8.0.1 (#1066)
- **mypy_stubs:** drop stale byte-parity twin wording from verifier_core shim (#1130)
- make production hydration license-gated and policy-addressed (#1118)
- render Bash command in quiet-mode tool panels (Closes #1108) (#1114)
- **daydream:** Consolidated: add delete_runs to archive index for hydration rerun pruning (#1101)
- Pin Daydream workflow installs to immutable release commits (#713) (#1100)
- make frozen corpus v2 directly consumable by Stage 0, SFT, and RFT (Closes #1081) (#1092)
- enforce corpus-v2 stack, repository, and profile output-share caps (#1090)
- degrade quality analysis on known-bad tree-sitter 0.26.0 install (fixes #1087) (#1089)
- carry repository and license identity and enforce C5/C8 in corpus v2 (Closes #1080) (#1088)
- **deps:** bump the dependencies group across 1 directory with 8 updates (#1070)
- **deps:** bump the dependencies group across 1 directory with 3 updates (#1069)
- **dev:** add repository-wide EditorConfig defaults (#1077)
- **deps:** bump mypy from 1.19.1 to 2.3.1 (#1073)
- **deps:** bump types-pyyaml from 6.0.12.20260518 to 6.0.12.20260815 (#1072)
- **deps:** bump rich from 14.3.1 to 15.0.0 (#1071)
- **deps:** drop github-actions Dependabot ecosystem; bump create-github-app-token to v3.2.0 manually (#1075)
- **deps:** bump actions/setup-python from 6.2.0 to 7.0.0 (#1064)
- **bug:** Make out-of-scope issue filing opt-in and prevent stale reverted-edit issues (#1062)
- **deps:** add dependabot config for four bounded weekly update ecosystems (#1060)
- Unify CLI backends behind an injectable subprocess and JSONL transport (#1053)
- add a deterministic per-finding adjudication queue and human-label workflow (#1054)
- deterministic reward-calibration tooling and artifact schema (#999) (#1036)
- project per-finding semantic labels and native-profile traces into corpus v2 (#1035)
- document DAYDREAM_GIT_TOKEN harvest clone auth (README + CLAUDE.md) (#1031)
- **benchmark:** remove obsolete implementation plan (#1026)
- remove generated audit artifact
- scrub credential-bearing Git remotes from archive manifests and Hub uploads (#988)
- bump version strings to 0.28.0 to match release v0.28.0
- **dev:** add fast pre-commit quality gate without weakening pre-push (#962)
- **scripts:** remove unused demo scripts and their regression test (issue #956) (#961)
- Route plan-anchor Git checks through the bounded Git operations seam (#955)
...
- **pr-review:** drop a bad diagram instead of the findings comment (Closes #1176) (#1177)
- **observability:** correct OTLP and GenAI contracts for HoneyHive, LangSmith, and generic exports (#1175)
- **archive:** honor upload_run_bundle's never-raises contract (#1174)
- **archive:** tier the bundle secret scan, unwedge dump rollback (#1173)
- **pr-review:** read diagram head evidence without a checkout (Closes #1167) (#1168)
- **tests:** write the fake codex readiness marker atomically (#1166)
- **artifacts:** P10 artifact visibility — sanctioned inputs, private storage, routed handoff (#1161)
- verify exact pushed commit against remote CI (#1157)
- record truthful phase lifecycles and overlap-aware timing (#1158)
- make annotation checkpoint recovery and final publication immutable (#1154)
- **benchmark:** share strict manifest validation and approval state (#1155)
- isolate improve audits from source storage and tools (#1153)
- preserve authorized supporting files through fix finalization (#1151)
- validate benchmark snapshot scope and base provenance (#1152)
- scope review posting to an explicit compatible repository context (#1149)
- save every active trajectory on SIGINT and SIGTERM (#1150)
- **benchmark:** strip finding markers from candidate projection (#1148)
- **codex:** archive parser diagnostics and classify tool outcomes (#1147)
- **benchmark:** anchor Harbor verifier to its asset directory (#1145)
- **rl:** reject non-canonical protected test paths (#1144)
- **observability:** handle stale tracing dependencies at initialization (#1143)
- **diagrams:** inline host artifacts for Codex read-only author turns (#1140)
- **harbor:** render_job_config requires credentials the selected provider/backend never uses (#1139)
- **backends:** bound codex xcrun resolve + thread-safe cached real-git resolver (#1138)
- **codex:** parse current file_change contract (Closes #1125) (#1136)
- preserve failure metadata on interrupted tool calls (#1126) (#1132)
- **backends:** reject negative osprey telemetry at the JSONL boundary (Closes #710) (#1099)
- **benchmark:** correct stale 12-key reward-payload claims in docs, tests, and _emit_reward docstring (Closes #991) (#1091)
- **adjudication:** complete materialization + publish-final gates (#1078) (#1085)
- **hydrate:** discover root-level Hugging Face sessions (#1074)
- **training:** annotation snapshot pipeline hardening (temporal gold gate, dedup, bundle checks) (#1055) (#1076)
- **harbor:** scoring canonicalization + verifier_core cleanup (issue #1004) (#1034)
- **tests:** make test helpers importable
- **benchmark:** reply-thread identity, stale-view spin, facts gate (#879)
- **benchmark:** project gold from authoring-time anchors, not GitHub's re-anchored commit_id/line/path (#996)
- **osprey:** bound oversized backend streams (#990)
- **training:** derive labels from reply meaning, not reply/merge presence (Closes #980) (#987)
- **benchmark:** harden claude-cli judge and verifier image build (Closes #965) (#978)
- **osprey:** harden JSONL and thinking event streams (#958)
- **improve:** discover pre-PEP-621 Python services (#963) (#964)
