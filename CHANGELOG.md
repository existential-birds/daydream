...
- **eval:** `analyze_shipped_duplication`'s pair rows carry `a_uid` / `b_uid`, so a shipped duplicate is traceable back to its originating per-stack record when the item retained a pre-merge identity (`""` for merge-agent-authored items, which is the common multi-stack case and not an error) (#1111)
