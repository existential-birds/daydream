...
- **deep:** `dropped-speculative.json` gains per-item `dropped_source_uids` beside `dropped_ids`/`dropped_uids`. `dropped_uids` answers "which object did the gate delete?" and is empty for every merge-agent item; the derivation answers "what was it made of?" and is populated on the main path, so the audit sidecar is no longer silent in the common multi-stack case (#1111)
